...
## Verification
- Settings contract checks for default, JSON key, and Reader category.
- Entry-decision tests for setting on/off, fresh entry, saved progress, text/start, cover-page, wrapper, and missing next spine.
- Cache-format/static checks for `coverPageHref` and cache-version bump.
